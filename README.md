...
Drop app screenshots, circle what's broken, say why, export the set.

Built for reviewing mobile app builds: drag in a batch of screenshots, mark them up in red,
talk your feedback instead of typing it, and export each one as a single image with the
...
| **Dictate** | Hit the mic (or `Space`). Live interim text shows as you talk; final text lands in the note. Editable by hand. |
| **Navigate** | `←` / `→`, the on-screen arrows, or the filmstrip. A red dot marks shots that already have a note. |
| **Export** | `Export` for the current shot, `Download all` for the set. |

## Stack

Vite + React + TypeScript. No backend, no accounts, no uploads — every image stays in the
